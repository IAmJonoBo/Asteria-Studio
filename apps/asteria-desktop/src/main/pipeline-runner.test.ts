...
describe("Pipeline Runner", () => {
  ...
  it("evaluateResults provides observations and recommendations", () => {
    const mockResult: PipelineRunnerResult = {
      ...
      pipelineResult: {
        runId: "run-test",
        runDir: path.join(os.tmpdir(), "asteria-runs", "run-test"),
        status: "success",
        pagesProcessed: 100,
        errors: [],
  ...
  it("evaluateResults flags normalization issues and fallback bleed/trim", () => {
    const evaluation = evaluateResults({
      ...
      pipelineResult: {
        runId: "run-metrics",
        runDir: path.join(os.tmpdir(), "asteria-runs", "run-metrics"),
        status: "success",
        pagesProcessed: 120,
        errors: [],
        metrics: {
  ...
  it("evaluateResults handles failed pipeline", () => {
    const evaluation = evaluateResults({
      ...
      pipelineResult: {
        runId: "run-fail",
        runDir: path.join(os.tmpdir(), "asteria-runs", "run-fail"),
        status: "error",
        pagesProcessed: 0,
  ...
  it("evaluateResults flags high variance", () => {
    ...
    const mockResult: PipelineRunnerResult = {
      ...
      pipelineResult: {
        runId: "run-variance",
        runDir: path.join(os.tmpdir(), "asteria-runs", "run-variance"),
        status: "success",
        pagesProcessed: 10,
        errors: [],
  ...
  it("evaluateResults flags large corpus and zero bounds", () => {
    ...
    const evaluation = evaluateResults({
      ...
      pipelineResult: {
        runId: "run-large",
        runDir: path.join(os.tmpdir(), "asteria-runs", "run-large"),
        status: "success",
        pagesProcessed: 150,
        errors: [],
        metrics: {
