...
const updateTemplateMeans = (template: TemplateBuilder, page: PageTemplateFeatures): void => {
  ...
  type MeanNumericKey = Exclude<keyof TemplateBuilder["mean"], "margins">;
  const update = (key: MeanNumericKey, value: number | undefined) => {
  ...
  const sidecarEntries = (
    await Promise.all(
      config.pages.map(async (page, index) => {
        ...
        const folioBandScore = computeFolioBandScore(elements, bookModel?.folioModel);
